...
def main():
    ...
    while True:
        ...
        # mask for yellow
        yellow_mask = cv2.inRange(hsv_img, (22, 150, 150), (30, 255, 255))

        contour_list, _ = cv2.findContours(yellow_mask, mode=cv2.RETR_EXTERNAL, method=cv2.CHAIN_APPROX_SIMPLE)
        # _, contour_list, _ = cv2.findContours(yellow_mask, mode=cv2.RETR_LIST, method=cv2.CHAIN_APPROX_SIMPLE)
        ...
        output_stream.putFrame(yellow_mask)
        print(x_list)
        print(y_list)
